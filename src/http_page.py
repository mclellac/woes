...
class HttpPage(Adw.PreferencesPage):
    ...
    def _prepare_request_headers(
        ...
        if user_agent:
            ...
            logger.info("No custom User-Agent; `requests` default will be used.")
        if use_akamai_pragma:
            ...
            logger.info("Akamai Pragma headers not included.")
        return initial_request_specific_headers, session_headers

    def _fetch_headers_task_thread_func(
        self, task: Gio.Task, source_object, task_data_arg: dict, cancellable: Optional[Gio.Cancellable]
    ):
        ...
        if custom_dns_server and not dns:
            logger.warning(
                "HttpPage: Custom DNS ('%s') configured, but dnspython missing for adapter.", custom_dns_server
            )
        ...
        logger.info(
            "HttpPage: CustomDNSAdapter mounted (DNS: '%s', SNI hint for IP URL: '%s').",
        ...
        try:
            if cancellable and cancellable.is_cancelled():
                task.return_new_error_literal(
                    GLib.quark_from_string(WOES_HTTP_ERROR_DOMAIN), HttpErrorType.CANCELLED.value, "Task cancelled."
                )
                return
            ...
            task.return_new_error_literal(
                GLib.quark_from_string(WOES_HTTP_ERROR_DOMAIN), HttpErrorType.TIMEOUT.value, msg
            )
            ...
            task.return_new_error_literal(
                GLib.quark_from_string(WOES_HTTP_ERROR_DOMAIN), HttpErrorType.CONNECTION_ERROR.value, msg
            )
        except requests.exceptions.RequestException as e:
            logger.warning("Task thread: RequestException for '%s': %s", url_to_fetch, e)
            msg = f"Request failed: {e}. Verify URL and network."
            task.return_new_error_literal(
                GLib.quark_from_string(WOES_HTTP_ERROR_DOMAIN), HttpErrorType.REQUEST_EXCEPTION.value, msg
            )
        except Exception:
            logger.exception("Task thread: Unexpected error for URL '%s':", url_to_fetch)
            msg = "Unexpected internal error during request."
            task.return_new_error_literal(
                GLib.quark_from_string(WOES_HTTP_ERROR_DOMAIN), HttpErrorType.GENERIC_UNEXPECTED.value, msg
            )
    ...
    def _process_http_response(
        ...
        try:
            ...
            task.return_new_error_literal(
                GLib.quark_from_string(WOES_HTTP_ERROR_DOMAIN), HttpErrorType.HTTP_ERROR.value, error_message
            )
    ...
    def _get_detailed_connection_error_message(self, exc: Exception, url: str) -> Optional[str]:
        ...
        if found_connection_refused:
            ...
            if parsed_url_scheme == "https":
                return "Connection Refused: The server at the HTTPS URL actively refused the connection. This might be an HTTP-only service. Consider trying with 'http://'."
            if parsed_url_scheme == "http":
                return "Connection Refused: The server at the HTTP URL actively refused the connection. The server might only support HTTPS or be down. Consider trying with 'https://'."
    ...
    def _fetch_headers_task_done_cb(self, _source_object, result: Gio.AsyncResult, _user_data):
        ...
        try:
            ...
            if isinstance(actual_list_of_responses, list):
                ...
                show_global_error(
                    self,
                    f"Failed to process task result (unexpected data structure: {type(actual_list_of_responses).__name__}). Please check logs.",
                )
                if hasattr(self, "http_entry_row") and self.http_entry_row:
    ...
    def _format_http_error(self, e: requests.exceptions.HTTPError) -> str:
        ...
        if status_code == 403:
            return f"403 Forbidden: Access to {e.request.url} denied."
        if status_code == 404:
            return f"404 Not Found: Resource at {e.request.url} not found."
        if status_code == 500:
            return f"500 Internal Server Error for {e.request.url}."
