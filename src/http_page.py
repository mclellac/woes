...
class HttpPage(Adw.PreferencesPage):
    ...
    http_entry_row = Gtk.Template.Child("http_entry_row")  # type: ignore
    http_apply_button = Gtk.Template.Child("http_apply_button")  # type: ignore
    http_host_header_row = Gtk.Template.Child(
        "http_host_header_row")  # type: ignore
    http_user_agent_row = Gtk.Template.Child(
        "http_user_agent_row")  # type: ignore
    http_pragma_switch_row = Gtk.Template.Child(
        "http_pragma_switch_row")  # type: ignore
    http_column_view = Gtk.Template.Child("http_column_view")  # type: ignore
    http_results_group = Gtk.Template.Child(
        "http_results_group")  # type: ignore
    clear_results_button = Gtk.Template.Child(
        "clear_results_button")  # type: ignore
    copy_results_button = Gtk.Template.Child()  # type: ignore
    http_status_row = Gtk.Template.Child()  # type: ignore
    http_status_spinner = Gtk.Template.Child()  # type: ignore
    ...
    def __init__(self, **kwargs: GObject.GObject):
        ...
        self.header_list_store = Gio.ListStore.new(HeaderItem)  # type: ignore
        selection_model = Gtk.MultiSelection.new(
            self.header_list_store)  # type: ignore
        self.http_column_view.set_model(selection_model)  # type: ignore
        if not self.http_column_view.get_columns():  # type: ignore
            header_name_factory = self._create_factory("key")
            header_value_factory = self._create_factory("value",
                                                        wrap_text=True)
            col_name = Gtk.ColumnViewColumn.new(
                "Header", header_name_factory)  # type: ignore
            col_value = Gtk.ColumnViewColumn.new(
                "Value", header_value_factory)  # type: ignore
            col_value.set_expand(True)  # type: ignore
            self.http_column_view.append_column(col_name)  # type: ignore
            self.http_column_view.append_column(col_value)  # type: ignore
        self._connect_signals()
        self._update_user_agent_model()
        self.settings.connect(
            "changed::custom-user-agents",
            lambda _s, _k: self._update_user_agent_model())
        if self.http_host_header_row:
            self._on_host_header_changed(
                self.http_host_header_row)  # type: ignore
        if self.http_user_agent_row:
            self._on_user_agent_changed(
                self.http_user_agent_row, None)  # type: ignore

        self.column_view_helper = Helper(
            widget=self.http_column_view,  # type: ignore
            parent_window=self.get_native())  # type: ignore

    def _connect_signals(self) -> None:
        """Connect signals for UI elements to their respective handlers."""
        self.http_entry_row.connect(  # type: ignore
            "entry-activated", self._on_entry_row_activated)
        self.http_apply_button.connect(  # type: ignore
            "clicked", self._on_entry_row_activated)
        self.http_pragma_switch_row.connect(  # type: ignore
            "notify::active", self._on_pragma_toggled)
        self.clear_results_button.connect(  # type: ignore
            "clicked", self._on_clear_results_clicked)
        if self.copy_results_button:
            self.copy_results_button.connect(  # type: ignore
                "clicked", self._on_copy_results_clicked)
        if self.http_host_header_row:
            self.http_host_header_row.connect(  # type: ignore
                "changed", self._on_host_header_changed)
        if self.http_user_agent_row:
            self.http_user_agent_row.connect(  # type: ignore
                "notify::selected-item", self._on_user_agent_changed)
    ...
    def _on_user_agent_changed(self, combo_row: Adw.ComboRow, _gparam: Optional[GObject.ParamSpec]) -> None:
        ...
        if isinstance(selected_item_obj, Gtk.StringObject):  # type: ignore
            ...
            effective_ua_value = self._ua_title_to_value_map.get(
                selected_title)
        ...
        if lines:
            ...
            try:
                ...
                if clipboard:
                    ...
                    logger.warning(
                        "Failed to get default clipboard for copying.")
            except Exception as e:
                logger.error(
                    "Error copying headers to clipboard: %s", e, exc_info=True)
        else:
    ...
    def _on_entry_row_activated(self, _widget: Gtk.Widget) -> None:
        ...
        if isinstance(selected_title_obj, Gtk.StringObject):
            ...
            user_agent_to_send = self._ua_title_to_value_map.get(
                selected_title)
        ...
        self._http_task_data_for_thread = {
            ...
            "custom_dns_server": (custom_dns_server
                                  if custom_dns_server else None),
        }
    ...
    def _fetch_headers_task_thread_func(
        ...
        # type: ignore # Unused (params retrieved from self._http_task_data_for_thread)
        ...
        try:
            ...
            if cancellable and cancellable.is_cancelled():  # Check again after fetch_headers returns
                task.return_new_error_literal(  # type: ignore
                    GLib.quark_from_string(
                        WOES_HTTP_ERROR_DOMAIN), HttpErrorType.CANCELLED.value, "Task cancelled after fetching."
                )
            else:
                task.return_value(processed_data)  # type: ignore
        except HttpRequestTimeoutError as e:
            logger.warning(
                "HttpPage Task: Timeout for '%s': %s", url_to_fetch, e)
            task.return_new_error_literal(  # type: ignore
                GLib.quark_from_string(
                    WOES_HTTP_ERROR_DOMAIN), HttpErrorType.TIMEOUT.value, str(e)
            )
        except HttpConnectionError as e:
            logger.warning(
                "HttpPage Task: ConnectionError for '%s': %s", url_to_fetch, e)
            task.return_new_error_literal(  # type: ignore
                GLib.quark_from_string(
                    WOES_HTTP_ERROR_DOMAIN), HttpErrorType.CONNECTION_ERROR.value, str(e)
            )
        except HttpProcessingError as e:  # Raised by HttpFetcher for HTTP 4xx/5xx errors
            logger.warning(
                "HttpPage Task: HttpProcessingError for '%s': %s", url_to_fetch, e)
            task.return_new_error_literal(  # type: ignore
                GLib.quark_from_string(
                    WOES_HTTP_ERROR_DOMAIN), HttpErrorType.HTTP_ERROR.value, str(e)
            )
        except HttpGenericRequestError as e:  # Other requests library errors
            logger.warning(
                "HttpPage Task: HttpGenericRequestError for '%s': %s", url_to_fetch, e)
            task.return_new_error_literal(  # type: ignore
                GLib.quark_from_string(
                    WOES_HTTP_ERROR_DOMAIN), HttpErrorType.REQUEST_EXCEPTION.value, str(e)
            )
        except HttpClientError as e:  # Catches other client errors including cancellation from HttpFetcher
            logger.warning(
                "HttpPage Task: HttpClientError for '%s': %s", url_to_fetch, e)
            # Check if it's a cancellation error from HttpFetcher
            ...
            logger.exception(
                "HttpPage Task: Unexpected generic error for URL '%s':",
                url_to_fetch)
            task.return_new_error_literal(
                GLib.quark_from_string(WOES_HTTP_ERROR_DOMAIN),
    ...
    def _fetch_headers_task_done_cb(
        ...
        Processes the result (header data or an exception reported by
        HttpFetcher via Gio.Task) and updates the UI. Re-enables UI elements.

        Args:
            _source_object (GObject.Object): The source object that initiated
                                             the task (unused).
            result (Gio.AsyncResult): The Gio.AsyncResult from the completed
                                      task.
            _user_data (object): User data passed with the callback (unused).
        """
        ...
        if task_being_processed is None:
            ...
            if hasattr(self, "http_entry_row") and self.http_entry_row and \
               not self.http_entry_row.get_sensitive():
                self.http_entry_row.set_sensitive(True)
            if (hasattr(self, "http_apply_button") and
                    self.http_apply_button and
        ...
        logger.info(
            "Processing task completion in _fetch_headers_task_done_cb.")
        try:
            propagate_result = task_being_processed.propagate_value()
            # actual_list_of_responses is now the direct result from
            # HttpFetcher if successful
            actual_list_of_responses: Optional[List[Dict[str, Any]]] = None

            # Check for list first, as it's the most direct expected type
            if isinstance(propagate_result, list):
                actual_list_of_responses = propagate_result
            # Heuristic for a namedtuple
            elif hasattr(propagate_result, '_asdict') and \
                    hasattr(propagate_result, '_fields'):
                logger.info("HttpPage: Received namedtuple-like object %s, "
                            "attempting to extract data.",
                            type(propagate_result))
                # Assuming the list is the first field, or a field named
                # 'result' or 'data'
                if 'result' in propagate_result._fields and \
                   isinstance(propagate_result.result, list):
                    actual_list_of_responses = propagate_result.result
                elif 'data' in propagate_result._fields and \
                        isinstance(propagate_result.data, list):
                    actual_list_of_responses = propagate_result.data
                elif propagate_result._fields and \
                    hasattr(propagate_result, propagate_result._fields[0]) and \
                    isinstance(
                        getattr(propagate_result, propagate_result._fields[0]),
                        list):
                    actual_list_of_responses = getattr(
                        propagate_result, propagate_result._fields[0])
                else:  # Fallback if fields are empty, not matching, or not a list
                    logger.error("HttpPage: namedtuple-like object detected, "
                                 "but could not extract List[Dict[str, Any]] "
                                 "data: %s", propagate_result)
                    # Ensure it's None if extraction fails
                    actual_list_of_responses = None
            elif isinstance(propagate_result, Gio.DBusCallFlags):
                # This was a workaround, ideally HttpFetcher's result is
                # directly a list or raises. If HttpFetcher returns list
                # directly, this might not be needed. For now, keeping it if
                # task.return_value() could still wrap.
                logger.debug("HttpPage: propagate_result is "
                             "Gio.DBusCallFlags, accessing .value")
                actual_list_of_responses = propagate_result.value
            # Generic fallback for a wrapper with a .value attribute
            elif hasattr(propagate_result, 'value'):
                logger.warning("HttpPage: Received wrapped object %s with "
                               ".value attribute.", type(propagate_result))
                actual_list_of_responses = propagate_result.value
            else:
                logger.error("HttpPage: Unexpected type from propagate_value: "
                             "%s", type(propagate_result))
                # This path implies an error that wasn't caught and converted
                # to GLib.Error by the thread func or HttpFetcher didn't
                # return a list as expected upon success.
                show_global_error(
                    self, "Unexpected result type from background task.")
                if hasattr(self, "http_entry_row") and self.http_entry_row:
                    self.http_entry_row.add_css_class("error")
                self._update_column_view_model(None)
                return  # Early exit

            # This block is reached if propagate_result was successfully
            # coerced or extracted.
            # Ensure actual_list_of_responses is a list before proceeding.
            if actual_list_of_responses is not None and \
               isinstance(actual_list_of_responses, list):
                logger.info("HttpPage: Successfully processed task result: %d "
                            "response stages.", len(actual_list_of_responses))
                processed_headers_for_store: list[HeaderItem] = []
                # Empty list is a valid success case (e.g. no data)
                if not actual_list_of_responses:
                    logger.info("HttpPage: Received empty list of responses.")
                    # Clear view or show "no data"
                    self._update_column_view_model(None)
                else:
                    for i, response_data_dict_item in \
                            enumerate(actual_list_of_responses):
                        # Ensure each item is a dict, as expected by
                        # HttpFetcher's contract
                        if not isinstance(response_data_dict_item, dict):
                            logger.error("HttpPage: Expected dict item in "
                                         "response list, got %s. Data: %s",
                        ...
                        response_data_dict: Dict[str, Any] = \
                            response_data_dict_item
                        url_display = \
                            f"URL: {response_data_dict.get('url', 'N/A')}"
                        status_code = response_data_dict.get('status_code',
                                                             'N/A')
                        response_type = response_data_dict.get('type',
                                                               'unknown')
                        status_display = \
                            (f"Status: {status_code} "
                             f"({str(response_type).capitalize()})")
                        processed_headers_for_store.append(
                            HeaderItem(key=url_display, value=status_display,
                                       is_special_row=True))
                        headers_for_this_response = \
                            response_data_dict.get("headers", {})
                        if isinstance(headers_for_this_response, dict):
                            for key, value in \
                                    headers_for_this_response.items():
                                processed_headers_for_store.append(
                                    HeaderItem(key=str(key), value=str(value),
                                               is_special_row=False))
                        else:
                            logger.warning("HttpPage: Headers data for "
                                           "response stage %d is not a dict: %s",
                                           i, headers_for_this_response)
                        if i < len(actual_list_of_responses) - 1:
                            processed_headers_for_store.append(
                                HeaderItem(key="--- Redirected To ---",
                                           value="", is_special_row=True))
                    self._current_header_items = processed_headers_for_store
                    self._update_column_view_model(
            ...
            # actual_list_of_responses is None or not a list after
            # attempted extraction
            else:
                logger.error("HttpPage: Failed to obtain a valid list of "
                             "responses. Value: %s", actual_list_of_responses)
                show_global_error(
                    self, "Failed to process data from background task.")
                if hasattr(self, "http_entry_row") and self.http_entry_row:
                    self.http_entry_row.add_css_class("error")
                self._update_column_view_model(None)
                self._set_loading_state(False,
                                        "Error: Failed to process data.")
        # Errors set by task.return_new_error_literal in
        # _fetch_headers_task_thread_func
        except GLib.Error as e:
            logger.warning("HttpPage: Task failed with GLib.Error (Domain: %s,"
                           " Code: %d, Message: %s)",
                           e.domain, e.code, e.message)
            display_message = e.message if e.message \
            ...
            self._set_loading_state(
                False, f"Error: {display_message.splitlines()[0]}")
        # Catch any other Python exceptions from this callback itself
        except Exception as e:
            logger.exception("HttpPage: Unexpected Python error in "
                             "_fetch_headers_task_done_cb:")
            ...
            self._update_column_view_model(None)
            self._set_loading_state(
                False, f"Error: {error_message.splitlines()[0]}")
        finally:
            # _set_loading_state(False, ...) called in success/error blocks
            # handles sensitivity and spinner. If no specific message for
            # success/error, it defaults to "Idle". If an error occurred, the
            # error message is set as status. If successful, a success message
            # should be set. The explicit calls to _set_loading_state in
            # try/except blocks handle this. This finally block ensures
            # controls are re-enabled if an unexpected exit happens, but the
            # status message should reflect the outcome from try/except.
            # If no specific status was set by success/error handlers (e.g. if
            # an error happened before status was set), and the task is done,
            # default to "Idle" or a generic "Finished".
            # Should always be true here if task is done
            if self.current_http_task is None:
                if self.http_status_row and \
                   self.http_status_row.get_subtitle() == "Fetching headers...":
                    # This means neither success nor specific error message
                    # was set for status_row
                    self._set_loading_state(False, "Idle - operation ended.")

    def _set_loading_state(self, active: bool, message: str = "Idle") -> None:
        """Sets the UI loading state (spinner, status message, sensitivity of
        input fields)."""
    ...
    def _on_pragma_toggled(self, _widget: Gtk.Switch, _gparam: GObject.ParamSpec) -> None:
        ...
        logger.debug("Akamai Pragma toggled: %s. Re-fetching if URL present.",
                     self.http_pragma_switch_row.get_active())  # type: ignore
    ...
    def _on_color_setting_changed(self, settings: Gio.Settings, key: str) -> None:
        ...
        if self._current_header_items:  # Re-populate to apply new colors
            logger.debug(
                "Re-populating ColumnView to apply new color changes.")
            self._update_column_view_model(self._current_header_items)

    def _update_user_agent_model(self) -> None:
        """Update the model for the User-Agent Adw.ComboRow.
        ...
        custom_ua_pairs: list[tuple[str, str]] = list(
            variant.unpack()  # type: ignore
            if variant and variant.get_type_string() == "a(ss)" else [])
        ...
        # Actual value for "None" selection
        ...
        self.http_user_agent_row.set_model(
            Gtk.StringList.new(display_titles))
        ...
        if current_selection_text and current_selection_text in display_titles:
            try:
                ...
                logger.warning("Error restoring User-Agent selection: '%s' "
                               "not found in new list.", current_selection_text)
                # Default to first if old selection is gone
                if display_titles:
                    self.http_user_agent_row.set_selected(0)
        # Default to first item if no prior or unmatchable selection
        elif display_titles:
            self.http_user_agent_row.set_selected(0)
        else:  # Model is empty
        ...
        logging.info("User-Agent dropdown model updated with %d titles.",
                     len(display_titles))
        ...
        def bind_func_internal(_factory: Gtk.SignalListItemFactory, list_item: Gtk.ListItem) -> None:
            ...
            if item.is_special_row:
                if attr_name == "key":  # For special rows, 'key' might hold combined info or just the primary part
                    ...
                    label.set_markup(
                        f"<b><span foreground='{self._special_row_color}'>"
                        f"{full_text}</span></b>")
                ...
                label.set_markup(
                    f"<b><span foreground='{color_to_use}'>"
                    f"{escaped_text}</span></b>")
