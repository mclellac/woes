...
class WebScanPage(Adw.PreferencesPage):
    ...
    def __init__(self, **kwargs):
        ...
        if self.results_scrolled_window:
            ...
            print(
                "ERROR: results_scrolled_window is None in __init__, cannot add GtkSource.View.")
        ...
        if self.source_view:
            ...
            if buffer:
                ...
                if language:
                    ...
                    logger.warning(
                        "GtkSource language '%s' not found. Syntax highlighting may not apply.", "text")

        self._apply_webscan_source_view_style()  # Initial application

        self.style_manager.connect(
            "notify::dark", self._on_webscan_source_style_settings_changed)
        self.settings.connect("changed::source-style-scheme",
                              self._on_webscan_source_style_settings_changed)
        ...
        if self.webscan_cancel_button:
            self.webscan_cancel_button.connect(
                "clicked", self._on_cancel_scan_clicked)
        if self.clear_results_button:
            self.clear_results_button.connect(
                "clicked", self._on_clear_results_clicked)
        if self.copy_results_button:
            self.copy_results_button.connect(
    ...
    def _on_webscan_source_style_settings_changed(self, _manager_or_settings, _param_spec_or_key):
        ...
        logger.info(
            "WebScanPage: Dark theme or source style scheme changed. Applying new style.")
    ...
    def __del__(self):
        ...
        if self.current_web_scan_cancellable and \
           not self.current_web_scan_cancellable.is_cancelled():
            logger.info(
                "WebScanPage being destroyed, cancelling ongoing Nikto scan.")
            ...
            if text_content:
                try:
                    ...
                    if clipboard:
                        ...
                        logger.info(
                            "Webscan results copied to clipboard successfully.")
                    else:
                        logger.warning(
                            "Failed to get default clipboard for copying webscan results.")
                except Exception as e:  # pylint: disable=broad-except # Clipboard operations can be unreliable
                    logger.error(
                        f"Error copying webscan results to clipboard: {e}", exc_info=True)
            else:
    ...
    def on_scan_button_clicked(self, _widget: Gtk.Button):
        ...
        logger.debug(
            f"WebScanPage scan button clicked. URL: '{self.url_entry.get_text()}'")
        ...
        if not is_valid_url(target_url, schemes=['http', 'https', 'ftp']):
            show_global_toast(
                self, "Invalid URL format. Please enter a valid URL.")
            main_window = self.get_native()
            if not (main_window and hasattr(main_window, 'show_toast')):
                show_global_error(
                    self, "Invalid URL format. Please enter a valid URL.")
        ...
        if self.current_web_scan_task and not self.current_web_scan_task.is_done():
            if self.current_web_scan_cancellable and not self.current_web_scan_cancellable.is_cancelled():
                logger.info(
                    "Cancelling previous web scan task before starting new one.")
                self.current_web_scan_cancellable.cancel()
        ...
        task = Gio.Task.new(self, self.current_web_scan_cancellable,
                            self._on_scan_task_done, None)  # type: ignore
        ...
        # Store params in instance variable
    ...
    def _run_scan_task_thread_func(self,
        ...
        if not scan_params:
            ...
            task.return_new_error_literal(GLib.quark_from_string(
                WEB_SCAN_ERROR_DOMAIN), WebScanErrorType.GENERIC.value, "Missing scan parameters in thread.")  # type: ignore
            return
        ...
        if target_url.startswith("ftp://"):
            logger.info(
                "FTP URL provided; Nikto might not directly support it. Attempting with http:// for the host part.")
            ...
            logger.info(
                "Prepending http:// to target URL for Nikto as scheme was missing or not http/https.")
        ...
        if maxtime_str:
            try:
                ...
                if maxtime_val > 0:
                    ...
                    logger.warning(
                        f"Invalid maxtime value '{maxtime_str}', must be positive. Ignoring.")
            except ValueError:
                logger.warning(
                    f"Invalid maxtime value '{maxtime_str}', not an integer. Ignoring.")

        tuning_options = []
        if cgi_vulns:
            # Corresponds to Nikto's "Misconfiguration / Default File"
            tuning_options.append('2')
        if interesting_content:
            # Corresponds to Nikto's "Interesting File / Seen in logs"
        ...
        if nikto_format_str and nikto_format_str not in ["default (text)", "txt (text)"]:
            ...
            # Nikto's default is text, -Format txt is redundant
        ...
        try:
            if cancellable.is_cancelled():
                task.return_new_error_literal(GLib.quark_from_string(
                    WEB_SCAN_ERROR_DOMAIN), WebScanErrorType.CANCELLED.value, "Scan cancelled before Nikto process start.")  # type: ignore
                return

            process = subprocess.Popen(
                nikto_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding="utf-8")
            # Store for potential external cancellation (e.g. __del__) - though __del__ might not run in main thread
            ...
            while True:
                if cancellable.is_cancelled():
                    logger.info(
                        "Nikto scan cancellation polling: Scan cancelled.")
                    if process.poll() is None:  # Check if process is still running
                        try:
                            ...
                            logger.warning(
                                "Nikto process did not terminate gracefully, killing.")
                            process.kill()
                        except Exception as e_term:  # pylint: disable=broad-except # Process termination can have diverse errors
                            logger.error(
                                f"Error terminating Nikto process: {e_term}")
                    task.return_new_error_literal(GLib.quark_from_string(
                        WEB_SCAN_ERROR_DOMAIN), WebScanErrorType.CANCELLED.value, "Scan cancelled by user.")  # type: ignore
            ...
            stdout_str, stderr_str = process.communicate() # noqa: F841 (used conditionally below)
            ...
            if stderr_str:
                ...
                logger.debug(
                    f"Attempting to parse Nikto output from stderr. Cleaned stderr string (first 500 chars): {cleaned_stderr_str[:500]}")
                ...
                if match:
                    ...
                    logger.info(
                        f"Regex matched tuple-like string in stderr. Raw content (first 200 chars): {nikto_output_raw_string[:200]}")
                    try:
                        # Reconstruct a valid Python string literal for ast.literal_eval to unescape.
                        string_to_evaluate = quote_char_for_literal_eval + \
                            nikto_output_raw_string + quote_char_for_literal_eval
                        actual_nikto_output = ast.literal_eval(
                            string_to_evaluate)
                        parsed_output_from_stderr = True
                        logger.info(
                            "Successfully parsed Nikto's primary output from stderr using regex and ast.literal_eval.")
                        if stdout_str:
                            logger.info(
                                f"Nikto stdout channel contained (will be ignored): '{stdout_str[:200]}...'")
                    except Exception as e:
                        logger.warning(
                            f"ast.literal_eval failed for extracted string: {e}. String (first 200 chars): {nikto_output_raw_string[:200]}", exc_info=True)
                else:
                    logger.info(
                        "Nikto stderr did not match tuple-like string pattern via regex. Trying original startswith/endswith check.")
                    # Fallback to original logic if regex doesn't match.
                    if cleaned_stderr_str.startswith("('") and cleaned_stderr_str.endswith("', '')"):
                        try:
                            parsed_content_tuple = ast.literal_eval(
                                cleaned_stderr_str)
                            if isinstance(parsed_content_tuple, tuple) and len(parsed_content_tuple) == 2 and \
                               isinstance(parsed_content_tuple[0], str) and parsed_content_tuple[1] == '':
                                ...
                                parsed_output_from_stderr = True
                                logger.info(
                                    "Successfully parsed Nikto's primary output from stderr using original startswith/endswith method.")
                                if stdout_str:
                                    logger.info(
                                        f"Nikto stdout channel contained (original method, will be ignored): '{stdout_str[:200]}...'")
                            else:
                                logger.warning(
                                    f"Original method: Nikto stderr matched start/end but internal structure was not as expected. Data: {cleaned_stderr_str[:200]}...")
                        except (SyntaxError, ValueError) as e:
                            logger.warning(
                                f"Original method: ast.literal_eval failed for cleaned_stderr_str: {e}. Data (first 200 chars): {cleaned_stderr_str[:200]}", exc_info=True)
                    else:
                        logger.info(
                            "Nikto stderr did not match any known tuple-like string pattern. It will be treated as plain text.")
            ...
            if process.returncode not in [0, 1]:
                # Ensure this line is not duplicated if it's outside the 'if stderr_str:' block
                error_output_detail = actual_nikto_output if parsed_output_from_stderr else (
                    stderr_str or stdout_str) # stdout_str and stderr_str are used here
                logger.error(
                    f"Nikto process finished with an unexpected error code {process.returncode}. Output/Stderr: {error_output_detail}")
            ...
            if parsed_output_from_stderr:
                # Parsed output in stdout slot, original stderr (now None)
                task.return_value((actual_nikto_output, None))
            else:
                task.return_value((stdout_str, stderr_str) # stdout_str and stderr_str are used here
                                  )
        except FileNotFoundError:
            logger.error("Nikto command not found. Ensure it's in PATH.")
            task.return_new_error_literal(GLib.quark_from_string(WEB_SCAN_ERROR_DOMAIN), WebScanErrorType.NIKTO_NOT_FOUND.value,
                                          "Nikto command not found. Please ensure it is installed and in your system's PATH.")  # type: ignore
        ...
        # A global timeout for the entire scan could be implemented by checking time in the polling loop.
            ...
            logger.exception(
                f"An unexpected error occurred during Nikto scan task: {e}")
            task.return_new_error_literal(GLib.quark_from_string(
                WEB_SCAN_ERROR_DOMAIN), WebScanErrorType.GENERIC.value, str(e))  # type: ignore
        finally:
    ...
    def _on_scan_task_done(self, _source_object: GObject.Object, result: Gio.AsyncResult, _user_data: object):  # type: ignore
        ...
        if self._current_webscan_params:
            target_url = self._current_webscan_params.get(
                "target_url", target_url)
        ...
        stdout: Optional[str] = None # noqa: F841 (Assigned but potentially not used if error occurs early)
        stderr: Optional[str] = None # noqa: F841 (Assigned but potentially not used if error occurs early)
        ...
        try:
            ...
            # Handle potential Gio.DBusCallFlags like wrapper
                ...
                logger.error(
                    f"Nikto scan for {target_url} returned unexpected result format: {type(returned_data)}")
                show_global_error(
                    self, "Scan returned unexpected data format.")  # type: ignore
                self._update_textview(
                    None, "Error: Scan returned unexpected data format.", is_error_message=True)
            ...
            if s_out is not None:
                if not isinstance(s_out, str):
                    logger.warning(
                        f"Nikto stdout was type {type(s_out)}, expected str. Converting. Value: {s_out}")
            ...
            if s_err is not None:
                if not isinstance(s_err, str):
                    logger.warning(
                        f"Nikto stderr was type {type(s_err)}, expected str. Converting. Value: {s_err}")
            ...
            if final_stdout is not None or final_stderr is not None:
                self._update_textview(
                    final_stdout, final_stderr, is_error_message=False)
                if self.webscan_status_action_row:
                    if final_stdout or final_stderr:  # Check if there's actual content
                        self.webscan_status_action_row.set_subtitle(
                            "Scan complete. See results below.")  # type: ignore
                    else:  # Both are None or empty strings
                        self.webscan_status_action_row.set_subtitle(
                            "Scan complete. No output received.")  # type: ignore
                ...
                if self.webscan_status_action_row and self.webscan_status_action_row.get_subtitle() == "Scanning...":  # type: ignore
                    self.webscan_status_action_row.set_subtitle(
                        "Error: Unexpected scan result format.")  # type: ignore
            ...
            if self.webscan_status_action_row:
                self.webscan_status_action_row.set_subtitle(
                    brief_user_message)  # type: ignore
            show_global_error(self, brief_user_message)  # type: ignore
            self._update_textview(
                None, detailed_output_for_textview, is_error_message=True)
        except Exception:  # Catch any other Python exceptions from this callback itself
            logger.exception(
                f"Unexpected Python error in _on_scan_task_done for {target_url}:")
            user_message = "An unexpected error occurred."
            if self.webscan_status_action_row:
                self.webscan_status_action_row.set_subtitle(
                    user_message)  # type: ignore
            show_global_error(self, user_message +
                              " Check logs for details.")  # type: ignore
            self._update_textview(
                None, f"Error: {user_message}", is_error_message=True)
        finally:
    ...
    def _update_textview(self, stdout_content: Optional[str], stderr_content: Optional[str], is_error_message: bool = False):
        ...
        if stderr_content:
            if is_error_message:  # This is a pre-formatted error message for the text view
                # Add newline before error block
        ...
        ) if self.results_scrolled_window else None
        if scroll_adj:
            scroll_adj.set_value(scroll_adj.get_upper() -
                                 scroll_adj.get_page_size())
    ...
    def trigger_scan(self):
        ...
        if self.scan_button and self.scan_button.get_sensitive():
            ...
            show_global_toast(
                self, "A scan is already in progress. Cancel it or wait.")  # type: ignore
        else:
            logger.warning(
                "Webscan scan button not available or not sensitive, cannot trigger scan.")
