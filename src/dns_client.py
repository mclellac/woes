...
class DnsResolverClient:
    ...
    def __init__(self, custom_dns_server: Optional[str] = None):
        ...
        Args:
            custom_dns_server (Optional[str]): Optional IP address of a
                                               custom DNS server.
        """
    ...
    def _lookup_record_internal(self, query_name_str: str, record_type_str: str) -> List[Dict[str, Any]]:
        ...
        Args:
            query_name_str (str): The domain name or reverse IP to query.
            ...
            for rdata in answer:
                record: Dict[str, Any] = {
                    ...
                    'ttl': (rdata.ttl if hasattr(rdata, 'ttl')
                            else answer.response.answer[0].ttl),
                ...
                if rdata.rdtype == dns.rdatatype.A:
                    ...
                    record['texts'] = [
                        s.decode('utf-8', 'replace') for s in rdata.strings]
    ...
    def resolve(self, domain_or_ip: str, record_type: str) -> List[Dict[str, Any]]:
        ...
        Args:
            ...
            record_type (str): The DNS record type string (e.g., "A", "MX",
                               "PTR"). If "PTR" is requested for an IP, it
                               handles reverse DNS.

        Raises:
            DnsClientError: and its subclasses for various DNS resolution
                            issues.
        ...
        # The `is_valid_ip` check should ideally be done by the caller to
        # ensure `domain_or_ip` is appropriate for the `record_type`.
        # However, `dns.reversename.from_address` will raise if it's not an IP.
        if record_type.upper() == "PTR":
            try:
                # Attempt to convert to reverse name only if it looks like an
                # IP. A simple check here can prevent
                # `dns.reversename.from_address` from raising an exception if
                # `domain_or_ip` is a hostname. A more robust IP validation
                # might be needed depending on expected inputs.
                is_ip = False
                try:
                    # Use a proper IP validation library
                ...
                # If it's not an IP, and PTR is requested, it will likely fail
                # or return empty, which is the correct behavior for a non-IP
                # PTR query.
            except dns.exception.SyntaxError as e:  # Raised by from_address
                logger.warning(
                    "DnsResolverClient: Syntax error converting '%s' for PTR "
                    "query: %s. Proceeding with original target.",
                    domain_or_ip, e)
                # Proceed with domain_or_ip as query_target, which might be
                # intentional for non-IP PTRs
