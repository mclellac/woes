...
def apply_source_style_scheme(
    ...
    # Existing initial log is good.
    logger.debug(
        f"style_utils.apply_source_style_scheme: Starting with scheme_manager: {scheme_manager}, buffer: {buffer}, source_style_scheme: '{source_style_scheme}'"
    )
    ...
    if source_style_scheme not in ["Adwaita", "Adwaita-dark"]:  # Case sensitive check
        ...
        logger.debug(f"style_utils.apply_source_style_scheme: Scheme '{source_style_scheme}' not Adwaita(-dark), trying lowercase '{source_style_scheme_lower}'")
    ...
    if scheme:
        ...
        if applied_scheme:
            ...
            logger.error( # Existing
                "style_utils.apply_source_style_scheme: Failed to apply the style scheme, even though scheme object was obtained."
            )
    else:
        logger.error( # Existing
            "style_utils.apply_source_style_scheme: Style scheme '%s' (tried '%s') not found.", original_scheme_name_for_log, source_style_scheme
        )
        ...
        if default_scheme:
            ...
            logger.debug("style_utils.apply_source_style_scheme: Applied fallback scheme=%s", default_scheme_id) # Existing
        else:
            logger.error( # Existing
...
def set_widget_visibility(visible: bool, *widgets):
    ...
    for i, widget in enumerate(widgets):
        if widget:
            ...
            logger.warning(f"style_utils.set_widget_visibility: Attempted to set visibility of widget {i}, but it is None") # Existing
